...
void *InputAnswerThread(int fd_keypad);
	void *CountKeypadThread(void); //Keypad 카운터
...
int key_timer=0; //keypad 타이머를 위한 변수
int res=0; //게임 결과
char question[10]={}; //문제
char answer[10]={}; //답
...
int main(){
	...
	pthread_create(&score_t,NULL,&ScoreThread,fd_seg);//스코어를 출력하는 스레드 생성
	for(stage = 1 ; stage <= 8 ; stage++){ //총 8스테이지까지 실행
		...
		if(alert_checker == 1){
			StageMain(fd_piezo, fd_text, fd_keypad, fd_dot, fd_led); //Main 게임 함수
...
int StageMain(int fd_piezo, int fd_text, int fd_keypad, int fd_dot, int fd_led){
	...
	for(i=0;i<10;i++){//매 stage마다 question 배열에 새로운 hex값 입력
		question[i]=hex_arr[rand() % 16];//16진수 중 하나를 question 배열에 입력
		answer[i]='\0';//스테이지 시작마다 answer 초기화
	}
	printf("question = %s\n",question);
	...
	pthread_create(&counter,NULL,&CountMainDotThread,NULL); //카운터 시작
...
void *PiezoBgmThread(int fd_piezo){
	...
	int val[] = { 0x03, 0x00, 0x03, 0x00, 0x03, 0x00, 0x05, 0x00,
			...
			0x04, 0x00, 0x03, 0x00, 0x07, 0x00, 0x06, 0x00,
			0x05, 0x00, 0x06, 0x00, 0x05, 0x00, 0x04, 0x00,
			0x11, 0x00, 0x11, 0x00, 0x07, 0x00, 0x07, 0x00,//두번째
			0x03, 0x00, 0x03, 0x00, 0x03, 0x00, 0x05, 0x00,
			0x04, 0x00, 0x03, 0x00, 0x07, 0x00, 0x06, 0x00,
			0x05, 0x00, 0x06, 0x00, 0x05, 0x00, 0x04, 0x00,
			0x03, 0x00, 0x03, 0x00, 0x03, 0x00, 0x05, 0x00,
	...
	int len[] = {150000, 1500, 150000, 1500, 150000, 1500, 150000, 1500,
			...
			150000, 3000, 500000, 150000, 150000, 3000, 500000, 50000,//두번째
			150000, 1500, 150000, 1500, 150000, 1500, 150000, 1500,
			150000, 1500, 150000, 1500, 150000, 1500, 150000, 1500,
			150000, 1500, 150000, 1500, 150000, 1500, 150000, 1500,
			150000, 1500, 150000, 1500, 150000, 1500, 150000, 1500,
...
void *InputAnswerThread(int fd_keypad){
	...
	pthread_create(&keypad_counter,NULL,&CountKeypadThread,NULL); //타이머 시작
...
//keypad 입력을 원할히 하기 위한 thread 함수
...
void *SuccessPiThread(int fd_piezo){
	int i;
		...
		for(i=0; i<sizeof(val);i++){
			...
			if(i==6) //thread가 꺼지지 않는 오류가 발생하여 반복문 마지막일 때 종료
				pthread_exit(NULL);
...
void *FailPiThread(int fd_piezo){
	int i;
		...
		for(i=0; i<sizeof(val);i++){
			...
			if(i==12) //thread가 꺼지지 않는 오류가 발생하여 반복문 마지막일 때 종료
				pthread_exit(NULL);
